// Guardas de inclusão para evitar múltiplas inclusões do cabeçalho
#ifndef BUSCA_GRAFOS_H
#define BUSCA_GRAFOS_H

#include <string> // Inclusão para poder usar strings

// Declaração da função de busca em largura no grafo G1
void bfs_g1(const std::string& inicio);

// Declaração da função de busca em profundidade no grafo G2
void dfs_g2(const std::string& inicio);

#endif // Fim da guarda de inclusão
